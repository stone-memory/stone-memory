...
type Props = {
  ...
  /** Categories minus the admin-hidden ones, resolved server-side. */
  initialVisibleCategories: ProjectCategory[]
}

/**
 * Portfolio arrives from the server so the gallery is in the initial HTML.
 *
 * Note both fallbacks are needed: useVisibleCategories() deliberately returns
 * [] until its store hydrates, which would filter every project out during SSR
 * and reproduce the very blank page this change fixes.
 */
export function ProjectsPageClient({ initialProjects, initialVisibleCategories }: Props) {
  ...
  const storeVisibleCats = useVisibleCategories()
  const visibleCats = storeVisibleCats.length > 0 ? storeVisibleCats : initialVisibleCategories
