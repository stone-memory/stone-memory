...
export const defaultAbout: Record<Locale, AboutContent> = {
  uk: {
    ...
    paragraphs: [
      "Stone Memory — виробництво з натурального каменю у Костополі на Рівненщині. Дві рівноцінні лінії: меморіальні комплекси та вироби для дому й саду — стільниці, підвіконня, каміни, сходи, бруківка.",
      ...
      "Даємо 5 років гарантії на все — фундамент, монтаж і сам камінь. Особистий менеджер від запиту до встановлення.",
    ],
...
export type AboutOverrides = Partial<Record<Locale, Partial<AboutContent>>>
type Overrides = AboutOverrides
...
export const useAboutStore = create<AboutState>()((set, get) => ({
  ...
  hydrate: async (initial) => {
    ...
    if (initial && typeof initial === "object") {
      set({ overrides: initial, hasHydrated: true })
      return
    }
    set({ loading: true })
    try {
...
export function useAbout(locale: Locale, initial?: Overrides | null): AboutContent {
  ...
  // До гідрації — серверні дані, якщо є, щоб SSR і перший кадр збігалися з адмінкою.
  const o = hasHydrated ? overrides[locale] : initial?.[locale]
