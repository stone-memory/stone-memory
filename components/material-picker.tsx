...
import { priceLevelLabel, rockFamilyLabel } from "@/lib/i18n/copy/common"
...
export function MaterialPicker({ stone, defaultEntry, onChange }: Props) {
  ...
  const c = PRODUCT_COPY[locale].picker
  ...
  const pickCustom = (text: string) => {
    ...
    onChange({ material: text, label: text || c.customLabel, price: base, isCustom: true })
  }
  ...
  const ConfirmButton = () => (
    ...
    >
      ...
      {preview.key === selected ? c.keep : c.choose}
    </button>
  )
    ...
    <section className="mt-8">
      ...
      >
        <button
          ...
          aria-label={c.inspectAria(current.name)}
        >
          <Image src={current.swatch} alt={c.surfaceAlt(current.name)} fill sizes="96px" className="object-cover" />
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            <span className="text-base font-medium leading-snug">{isCustom ? customText || c.customLabel : current.name}</span>
            {!isCustom && current.key === defaultEntry.key ? (
              <span className="text-[10px] uppercase tracking-wider text-muted-foreground">{c.inPhoto}</span>
            ) : null}
          </div>
          {!isCustom ? (
            <p className="mt-0.5 text-xs text-muted-foreground">
              {rockFamilyLabel(current.rock, locale)} · {priceLevelLabel(current.priceLevel, locale)}
              {priceFor(current) ? ` · ${formatPrice(priceFor(current)!)}` : ""}
            </p>
          ) : (
            <p className="mt-0.5 text-xs text-muted-foreground">{c.customPriceNote}</p>
          )}
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        {c.note(defaultEntry.name, STONE_GUIDE.length)}
        {!isCustom && selected !== defaultEntry.key ? c.priceApprox : ""}
      </p>
      ...
      <details className="mt-2 rounded-xl border border-foreground/10 p-3" open={isCustom}>
        <summary className="cursor-pointer text-sm font-medium">{c.customSummary}</summary>
        ...
        <Dialog.Portal>
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-foreground/10 px-4 py-3 md:px-6 md:py-4">
              <div>
                <Dialog.Title className="text-base font-semibold tracking-tight-custom md:text-xl">{c.dialogTitle}</Dialog.Title>
                <Dialog.Description className="mt-0.5 text-xs text-muted-foreground md:text-sm">
                  {c.dialogLead(STONE_GUIDE.length)}
                </Dialog.Description>
              </div>
              <Dialog.Close
            ...
            <div className="min-h-0 flex-1 overflow-y-auto md:grid md:grid-cols-[1.05fr_1fr] md:overflow-hidden">
              <div ref={previewRef} className="flex flex-col gap-3 p-4 md:gap-4 md:overflow-y-auto md:border-r md:border-foreground/10 md:p-6">
                ...
                <div>
                  <div className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
                    ...
                    <span className="rounded-full bg-foreground/5 px-2 py-0.5 text-[11px] text-muted-foreground md:text-xs">{rockFamilyLabel(preview.rock, locale)}</span>
                    <span className="text-[11px] text-muted-foreground md:text-xs">{priceLevelLabel(preview.priceLevel, locale)}</span>
                  </div>
                  ...
                  {priceFor(preview) ? (
                    <p className="mt-2 text-base font-semibold tracking-tight-custom">
                      ...
                      <span className="ml-2 text-xs font-normal text-muted-foreground">{c.thisModelInStone}</span>
                    </p>
                  ) : null}
              ...
              <div role="radiogroup" aria-label={c.gridAria} className="border-t border-foreground/10 p-4 md:min-h-0 md:overflow-y-auto md:border-t-0 md:p-6">
                {groups.map((g) => (
                  <div key={g.rock} className="mb-5 last:mb-0">
                    <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                      {rockFamilyLabel(g.rock, locale)} · {g.items.length}
                    </p>
